...
if ($_POST['act'] == "save_karyawan"):
	$sql_cek = "SELECT * FROM [db_hris].[dbo].[table_karyawan_draft] WHERE nik = '" . trim($_POST['nik']) . "'";
	$stmt_cek = $sqlsrv_hris->query($sql_cek);
	$num_cek = $sqlsrv_hris->num_rows($stmt_cek);
	$sql = "";
	if ($num_cek > 0):
		$sql = "UPDATE [db_hris].[dbo].[table_karyawan_draft] set 
		...
		$sql = "INSERT INTO [db_hris].[dbo].[table_karyawan_draft]
				(
					[nama_pt]
					,[entity]
					,[depo]
...
elseif ($_POST['act'] == "save_foto"):
	...
	if ($_POST['total_foto'] > 0):
		$sql_cek = "select " . $_POST['jenis'] . " from [db_hris].[dbo].[table_karyawan_foto_draft] where nik = '" . trim($_POST['nik']) . "'";
		$stmt_cek = $sqlsrv_hris->query($sql_cek);
		...
		if ($num_row > 0):
			$sql = "update [db_hris].[dbo].[table_karyawan_foto_draft] set " . $_POST['jenis'] . " = '" . trim($nama_foto) . "' where nik = '" . trim($_POST['nik']) . "'";
		else:
			$sql = "insert into [db_hris].[dbo].[table_karyawan_foto_draft] 
				(
					[nik]
					," . $_POST['jenis'] . "
				) 
