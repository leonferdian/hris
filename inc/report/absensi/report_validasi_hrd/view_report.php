...
for ($day = 1; $day <= $numDaysInMonth; $day++) {
    ...
    $columns[] = "COUNT(CASE WHEN DATEPART(day, date_absen) = '$day' AND [validate_date] IS NULL THEN pin END) AS not_validadted_tgl_$dayFormatted";
    $columns[] = "COUNT(CASE WHEN DATEPART(day, date_absen) = '$day' AND [validasi_hrd] = 1 THEN pin END) AS validated_tgl_$dayFormatted";
    $columns[] = "COUNT(CASE WHEN DATEPART(day, date_absen) = '$day' AND [validasi_hrd] = 0 THEN pin END) AS reject_tgl_$dayFormatted";
...
<div class="table-header">
    <div class="pull-right">
        <a class="btn btn-white btn-xs btn-success"
            onclick="export_excel('table_absensi_report', 'Report Validasi Absensi HRD-<?php echo date('Ymdhis'); ?>')"><i
        ...
        <thead>
            <tr>
                ...
                for ($day = 1; $day <= $numDaysInMonth; $day++) {
                    ...
                    echo '<th class="center" style="border-right: #000 1px solid;" colspan="4">' . $dayFormatted . '</th>';
                }
                ?>
            </tr>
            <tr>
                ...
                for ($day = 1; $day <= $numDaysInMonth; $day++) {
                    ...
                    echo '<th class="center">Reject</th>';
                ...
                while ($row = $sqlsrv_hris->fetch_array($stmt_mesin)):
                    ...
                    <tr>
                        ...
                        for ($day = 1; $day <= $numDaysInMonth; $day++) {
                            $dayFormatted = str_pad($day, 2, '0', STR_PAD_LEFT);
                            $color = "";
                            if ($row['total_tgl_' . $dayFormatted] > 0) $color = "background-color: #ff00003d;";
                            echo '<td class="center"><a target="_blank" href="?sm=report_validasi_hrd&act=detail&validate=&depo='.$row['depo'].'&tgl='.$year.'-'.$month.'-'.$dayFormatted.'">' . $row['not_validadted_tgl_' . $dayFormatted] . '</a></td>';
                            echo '<td class="center"><a target="_blank" href="?sm=report_validasi_hrd&act=detail&validate=1&depo='.$row['depo'].'&tgl='.$year.'-'.$month.'-'.$dayFormatted.'">' . $row['validated_tgl_' . $dayFormatted] . '</a></td>';
                            echo '<td class="center"><a target="_blank" href="?sm=report_validasi_hrd&act=detail&validate=0&depo='.$row['depo'].'&tgl='.$year.'-'.$month.'-'.$dayFormatted.'">' . $row['reject_tgl_' . $dayFormatted] . '</a></td>';
                            echo '<td class="center" style="border-right: #000 1px solid; '.$color.'"><a target="_blank" href="?sm=report_validasi_hrd&act=detail&validate=0&depo='.$row['depo'].'&tgl='.$year.'-'.$month.'-'.$dayFormatted.'">' . $row['total_tgl_' . $dayFormatted] . '</a></td>';
                        }
                        ?>
                        <!-- <td class="center">
